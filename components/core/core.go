...
import (
	...
	// _ "github.com/v2fly/v2ray-core/v5/proxy/hysteria2" // TODO: erroneous
...
var GLOBAL_INSTANCE *core.Instance

type NewLogHandler struct {
}

func (h *NewLogHandler) Handle(msg log.Message) {
	switch msg := msg.(type) {
	case *log.AccessMessage:
		WriteAccessLog(serial.ToString(msg.From), serial.ToString(msg.To), serial.ToString(msg.Detour), msg.Status == log.AccessAccepted)

	case *log.GeneralMessage:
		WriteGeneralLog(msg.Severity, serial.ToString(msg.Content))
...
func DoStart(configBytes string) error {
	config, err := core.LoadConfig("jsonv5", []byte(configBytes))
	if err != nil {
		return err
	}

	GLOBAL_INSTANCE, err = core.New(config)
	if err != nil {
	...
	log.RegisterHandler(&NewLogHandler{})

	WriteLog("V2Ray instance created, version: " + core.Version())

	err = GLOBAL_INSTANCE.Start()
	if err != nil {
		return err
	}
...
func DoClose() {
	...
	GLOBAL_INSTANCE = nil
	WriteLog("V2Ray instance closed")
}

func main() {
	WriteLog("This is a shared library, not a standalone program.")
}
